...
class GaussianPolicy(MLP):
    def __init__(self, 
        ...
        self.output_limit = output_limit
        self.log_std = np.ones(output_size, dtype=np.float32) * -0.5 * output_limit
    ...
    def forward(self, x, pi=None, use_pi=True):
        ...
        mu_clipped = mu.clamp(-self.output_limit, self.output_limit)
        
        # This is the action for training
        pi_clipped = pi.clamp(-self.output_limit, self.output_limit)
        return mu_clipped, std, pi_clipped, log_pi
