...
class Agent(object):
   ...
   def __init__(self,
      ...
      self.old_policy = GaussianPolicy(self.obs_dim, self.act_dim, self.act_limit, output_activation=F.tanh).to(self.device)
   ...
   def train_model(self):
      ...
      for _ in range(self.train_vf_iters):
         ...
         rand_ids = torch.randperm(batch_size, device=self.device)
         
         # Loop over the data in minibatches
         for start in range(0, batch_size, minibatch_size):
            end = start + minibatch_size
            mb_ids = rand_ids[start:end]

            # Get the data for this minibatch
            mb_obs = obs[mb_ids]
            mb_ret = ret[mb_ids]

            # Prediction V(s)
            v = self.vf(mb_obs).squeeze(1)
      ...
      if self.args.algo == 'npg':
         ...
         for i in range(self.backtrack_iter):
            ...
            if kl < self.delta and improve_condition > self.backtrack_alpha:
               # print('Accepting new params at step %d of line search.'%i) # Silenced for cleaner output
               self.backtrack_iters.append(i)
               break

            if i == self.backtrack_iter-1:
               # print('Line search failed! Keeping old params.') # Silenced for cleaner output
               self.backtrack_iters.append(i)
